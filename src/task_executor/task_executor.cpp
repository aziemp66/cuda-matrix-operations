...
bool executeTask(const char* taskStr, int TPB, int n) {
  ...
  if (!taskName || !platform || !type) {
    printf("Error: Invalid task format '%s'. Expected format: {taskname}_{platform}_{type}\n", taskStr);
  ...
  if (strcmp(taskName, "vectoradd") == 0) {
    ...
    if (strcmp(platform, "cpu") == 0) {
      if (strcmp(type, "float") == 0) {
        MatrixMultTiledExampleCPUFloat(TPB, n);
        return true;
      } else if (strcmp(type, "double") == 0) {
        MatrixMultTiledExampleCPUDouble(TPB, n);
        return true;
      }
    } else if (strcmp(platform, "gpu") == 0) {
      if (strcmp(type, "float") == 0) {
...
void printTaskUsage() {
  printf("  Task name must be one of: vectoradd, matrixadd, matrixmultnaive, matrixmulttiled\n");
